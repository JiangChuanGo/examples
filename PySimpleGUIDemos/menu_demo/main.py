...
from weather_img import get_path
...
def update_city_weather(window, city):
    ...
    weather_text_widget = window["-WEATHER-TEXT-"]

    filename = get_path(weather)

    # 将天气更新到输入框

    weather_img_widget.Update(filename = filename)
    weather_text_widget.Update(weather)
...
# 菜单栏
menu_def = [['File', ['Open',['File1', 'File2', ['.txt', '.pdf']], 'Save', 'Exit',]],
...
layout = [[sg.Menu(menu_def)],
           ...
           [sg.Text("", size =(10, 1), key = "-WEATHER-TEXT-")],
...
# 更新初始城市天气，也就是北京市的天气
update_city_weather(window, province_city[provinces[0]][0])
...
while True:
    ...
    if event == "-PROVINCE-":
        ...
        # 更新显示天气
        update_city_weather(window,  city)
